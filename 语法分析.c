...
int get_word(FILE *source, FILE *intermediate_code) {
    ...
    int i;
    for (i = 0; i < 88; i++) { word[i] = (char) set[i]; }
    int _type = type;
    type = jump = 0;
    printf("%s\n", word);
...
void _stmt_sequence(FILE *source, FILE *intermediate_code) {
    ...
    if (!strcmp(word, "end") || !strcmp(word, "else") || !strcmp(word, "until")) { //处理空规则
        ...
        return;
                       ...
                       ";")) { //这里的";"并没有作用于_type之这也是为什么会有_type异常之bug，所以有go_on_1的地方都应该有_type。_type异常之bug困扰了己一天，这种bug是由于对go_on_1和_type控制机制掌握得不够全面而导致的，值得谨记
        if (go_on_1) { _type = get_word(source, intermediate_code); }
        go_on_1 = 1;
...
void _exp(FILE *source, FILE *intermediate_code) {
    ...
    if (!strcmp(word, "<") || !strcmp(word, "=")) {
        ...
        ListInsert_Pro(1, &Terminal_Symbol, ">< ");
        simple_exp(source,
                   intermediate_code); //词法分析把底层文法分析透明化了。之前这里把simple_exp错写成_simple_exp，则产生了莫名其妙的错误，这种拼写错误其实比逻辑错误更棘手之毕竟下意识中并没有写错，而只有突破界限意识到不是算法问题而是拼写问题代码问题什么才能修复bug。
    }
