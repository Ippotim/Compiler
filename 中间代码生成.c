        ...
        if (p->data == '{') {
            if (Match == 0) {
                ...
                do {
                    if (p->next->data == '}') {
                        break;
                    } //处理语法分析结束标志之空字符串之在一级中间代码中就是"{}"。本来没有这句之这是导致段错误的一个bug，由于在修复语法分析设计上的bug前己就发现了这个bug，所以当修复完语法分析设计上的bug发现还有段错误后己就敏锐地把目光投向了这里，果然加了这句二级代码生成就大功告成了
                    p = p->next;
                    No++;
...
void genThirdStepCode(LinkList SecondStepCode, LinkList *ThirdStepCode) {
    ...
    while (p) {
        ...
        if (strcmp(prefix, ";><><") != 0) {
            if (strcmp(prefix, "><;") != 0) {
                if (strcmp(prefix, "><;") != 0 && strcmp(prefix, "><><") != 0 && strcmp(prefix, "NULL") != 0 &&
                    strcmp(prefix, "><") != 0 && strcmp(prefix, "") != 0) { //顺便在此删除空函数名前缀式
...
void toSet(LinkList ThirdStepCode, char **fuc, int *Begin, int *End) //从三级中间代码中的各前缀式提取数据，这里的fuc指针果然需要引用，其实参还不能被定义为数组
{
    ...
    *fuc = prefix;
    ...
    number[i] = '\0';
    *Begin = (int) strtol(number, NULL, 10);
    p = p->next;
    ListDelete(&ThirdStepCode);
    i = 0;
    while (p->data != ']') {
...
char *toRegister(const char *r) //局部模型对接引擎或者说狭义模型对接模型。使用转换的原因是要用表达式层级之level选中寄存器，这样就要把寄存器存入广义变量之编制化
{
            ...
            int ExpEnd) //这里有一个难点，一般式需要存储三个Exp的值，而Exp的值是由同一个函数确定存储位置的，这似乎陷入绝路了，当然静态变量是一条活路，或者也可以增加参数之调用genExp()时可以分配不同的参数，这些都是建设性的解决方案。这里采用第三种方案，之r[0]、r[1]用来提取factor的值，r[2]、r[3]、r[4]分别用来存储三个Exp的值之作为参数用来存储公共Exp返回值之，r[5]用来存储第一个根Exp的值，之result用来指定genExp()用来存储Exp值的寄存器编号。不过现在认为选择另一种方案会更好，之采用汇编栈思想伪动态拓展寄存器数量，这样也能更自然地配合递归，这实在是无与伦比的绝妙之精妙绝伦。
{
    ...
    if (!strcmp(fuc, "(><)")) {
        ...
        //else return;
        /*[No++](save,/,/)*/
        ListInsert_Pro(4, FourthStepCode, "[");
        sprintf(_No, "%d", No++);
        ListInsert_Pro(4, FourthStepCode, _No);
        ListInsert_Pro(4, FourthStepCode, "]");
        ListInsert_Pro(4, FourthStepCode, "(save,/,/)");
        ...
        ListInsert_Pro(4, FourthStepCode, _No);
        ListInsert_Pro(4, FourthStepCode, "]");
        sprintf(_No, "r[%d]", level);
        sprintf(all, "(:=,[exp],%s)", toRegister(_No));
        ListInsert_Pro(4, FourthStepCode, all);
        /*[No++](restore,/,/)*/
        ListInsert_Pro(4, FourthStepCode, "[");
        sprintf(_No, "%d", No++);
        ListInsert_Pro(4, FourthStepCode, _No);
        ListInsert_Pro(4, FourthStepCode, "]");
        ListInsert_Pro(4, FourthStepCode, "(restore,/,/)");
        /*[No++](:=,r[0],[exp])*/
        ListInsert_Pro(4, FourthStepCode, "[");
        sprintf(_No, "%d", No++);
        ListInsert_Pro(4, FourthStepCode, _No);
        ListInsert_Pro(4, FourthStepCode, "]");
        sprintf(all, "(:=,%s,[exp])", toRegister("r[0]"));
        ListInsert_Pro(4, FourthStepCode, all);
    } else {
        if (isalpha(fuc[0])) {
            /*[No++](:=,r[0],[fuc])*/
            ...
            ListInsert_Pro(4, FourthStepCode, _No);
            ListInsert_Pro(4, FourthStepCode, "]");
            sprintf(_No, "Variable%d", st_lookup(fuc));
            sprintf(all, "(:=,%s,[%s])", toRegister("r[0]"), _No);
            ListInsert_Pro(4, FourthStepCode, all);
            ...
            ListInsert_Pro(4, FourthStepCode, _No);
            ListInsert_Pro(4, FourthStepCode, "]");
            sprintf(all, "(:=,%s,%s)", toRegister("r[0]"), fuc);
            ListInsert_Pro(4, FourthStepCode, all);
        }
        if (ThirdStepCode->next) { toSet(ThirdStepCode, &fuc, &Begin, &End); }
    ...
    level = 0;
    if ((!strcmp(fuc, "*><><") || !strcmp(fuc, "/><><") || !strcmp(fuc, "+><><") || !strcmp(fuc, "-><><") ||
         !strcmp(fuc, "<><") || !strcmp(fuc, "=><")) &&
        Begin < ExpEnd) { //原来复制粘贴的时候多了些'('，这样就导致程序逻辑失控了，只是己还以为是正确的，这种错误是很难发现的
        /*[No++](:=,r[1],r[0])*/
        ListInsert_Pro(4, FourthStepCode, "[");
        sprintf(_No, "%d", No++);
        ListInsert_Pro(4, FourthStepCode, _No);
        ListInsert_Pro(4, FourthStepCode, "]");
        sprintf(_No, "(:=,%s,", toRegister("r[1]"));
        sprintf(all, "%s)", toRegister("r[0]"));
        ListInsert_Pro(4, FourthStepCode, _No);
        ListInsert_Pro(4, FourthStepCode,
                       all); //本来是sprintf(all,"(:=,%s,%s)",toRegister("r[1]"),toRegister("r[0]"));之sprintf()里的两个toRegister()返回的字符串一样，这不可能，应该是函数处理参数是从右到左处理，先从左到右依次调用这里的两个toRegister()然后再统一取返回值，所以两者会一样
    } else { return; }
    ...
    while ((!strcmp(fuc, "*><><") || !strcmp(fuc, "/><><")) && Begin < ExpEnd) {
        ...
        ListInsert_Pro(4, FourthStepCode, _No);
        ListInsert_Pro(4, FourthStepCode, "]");
        sprintf(_No, "%s", toString(_fuc[0]));
        sprintf(all, "(%s,%s,[exp])", _No, toRegister("r[1]"));
        ListInsert_Pro(4, FourthStepCode, all);
    }
    level = 1;
    if ((!strcmp(fuc, "+><><") || !strcmp(fuc, "-><><") || !strcmp(fuc, "<><") || !strcmp(fuc, "=><")) &&
        ...
        ListInsert_Pro(4, FourthStepCode, _No);
        ListInsert_Pro(4, FourthStepCode, "]");
        sprintf(_No, "(:=,%s,", toRegister("r[2]"));
        sprintf(all, "%s)", toRegister("r[1]"));
        ListInsert_Pro(4, FourthStepCode, _No);
        ListInsert_Pro(4, FourthStepCode,
                       all); //这里本来错写成R[1]则导致莫名其妙的错误，这种错误是很难发现的之要进行自我怀疑否定。类似的错误还有似修改程序后没有保存或没有编译或编译时有点小毛病没通过而保留了上次编译通过的可执行文件。
    } else { return; }
    ...
    while ((!strcmp(fuc, "+><><") || !strcmp(fuc, "-><><")) && Begin < ExpEnd) {
        ...
        ListInsert_Pro(4, FourthStepCode, "]");
        sprintf(_No, "%s", toString(_fuc[0]));
        sprintf(all, "(%s,%s,[exp])", _No, toRegister("r[2]"));
        ListInsert_Pro(4, FourthStepCode, all);
    }
    level = 2;
    if ((!strcmp(fuc, "<><") || !strcmp(fuc, "=><")) && Begin < ExpEnd) {
        ...
        ListInsert_Pro(4, FourthStepCode, _No);
        ListInsert_Pro(4, FourthStepCode, "]");
        sprintf(_No, "(:=,%s,", toRegister("r[3]"));
        sprintf(all, "%s)", toRegister("r[2]"));
        ListInsert_Pro(4, FourthStepCode, _No);
        ListInsert_Pro(4, FourthStepCode, all);
    } else { return; }

    if ((!strcmp(fuc, "<><") || !strcmp(fuc, "=><")) && Begin <
                                                        ExpEnd) { //这里原来是if((!strcmp(fuc,"<><><")||!strcmp(fuc,"=><><"))&&Begin<ExpEnd)之记错了语法设置，所以出错了让己好找。当然这里用while也可以
        char _fuc[22];
        sprintf(_fuc, "%s", fuc);
        int tempEnd = End;
        if (ThirdStepCode->next) { toSet(ThirdStepCode, &fuc, &Begin, &End); }
        ...
        ListInsert_Pro(4, FourthStepCode, "]");
        sprintf(_No, "%s", toString(_fuc[0]));
        sprintf(all, "(%s,%s,[exp])", _No, toRegister("r[3]"));
        ListInsert_Pro(4, FourthStepCode, all);
        //break;
    }
    level = 3;
...
void genStmt(LinkList ThirdStepCode, LinkList *FourthStepCode, int StmtEnd) {
    ...
    while (Begin < StmtEnd) {
        if (!strcmp(fuc, "if><then><><")) {
            ...
            ListInsert_Pro(4, FourthStepCode, "]");
            sprintf(_No, "r[%d]", level);
            sprintf(all, "(jz,%s,", toRegister(_No));
            ListInsert_Pro(4, FourthStepCode, all);
            sprintf(_No, "%d)", A);
            ListInsert_Pro(4, FourthStepCode, _No);
            No++;
            ...
            ListInsert_Pro(4, FourthStepCode, _No);
            ListInsert_Pro(4, FourthStepCode, "]");
            ListInsert_Pro(4, FourthStepCode, "(jmp,/,");
            sprintf(_No, "%d", A);
            ListInsert_Pro(4, FourthStepCode, _No);
            ListInsert_Pro(4, FourthStepCode, ")");
            No++;
            ...
            ListInsert_Pro(4, FourthStepCode, _No);
            ListInsert_Pro(4, FourthStepCode, "]");
            ListInsert_Pro(4, FourthStepCode, "(StackIn,/,");
            sprintf(_No, "r[%d]", temp_level);
            sprintf(all, "%s)", toRegister(_No));
            ListInsert_Pro(4, FourthStepCode, all);
            ...
            ListInsert_Pro(4, FourthStepCode, "[");
            sprintf(_No, "%d",
                    B);/*这里本来是sprintf(_No,"%d",No++);但由于sprintf()是先从右到左把所有作为参数的函数调用调用完毕再采集参数，所以这里不能No++*/ListInsert_Pro(
                    4, FourthStepCode, _No);
            ListInsert_Pro(4, FourthStepCode, "]");
            ListInsert_Pro(4, FourthStepCode, "(nop,/,/)");
            ...
            ListInsert_Pro(4, FourthStepCode, "]");
            sprintf(_No, "r[%d]", 0);
            sprintf(all, "(StackOut,/,%s)", toRegister(_No));
            ListInsert_Pro(4, FourthStepCode, all);
            No++;
            ...
            ListInsert_Pro(4, FourthStepCode, "[");
            sprintf(_No, "%d",
                    No);/*这里本来是sprintf(_No,"%d",No++);但由于sprintf()是先从右到左把所有作为参数的函数调用调用完毕再采集参数，所以这里不能No++*/ListInsert_Pro(
                    4, FourthStepCode, _No);
            ListInsert_Pro(4, FourthStepCode, "]");
            ListInsert_Pro(4, FourthStepCode, "(nop,/,/)");
            No++;
            ...
            ListInsert_Pro(4, FourthStepCode, "]");
            sprintf(_No, "r[%d]", level);
            sprintf(all, "(jnz,%s,", toRegister(_No));
            ListInsert_Pro(4, FourthStepCode, all);
            sprintf(_No, "%d)",
                    No);/*printf("OK\n"); 之前是sprintf(_No,"%s)",No++);之段错误，之No是整型而不是指针，%s就是将之用作指针自然段错误，所以出现段错误时应注意下是否格式错误之尤其多次使用复制粘贴*/ListInsert_Pro(
                    4, FourthStepCode, _No);
            /*[C](jmp,/,A)*/
            ListInsert_Pro(4, FourthStepCode, "[");
            sprintf(_No, "%d", C);
            ...
            ListInsert_Pro(4, FourthStepCode, _No);
            ListInsert_Pro(4, FourthStepCode, "],");
            ListInsert_Pro(4, FourthStepCode, toRegister("r[0])"));
            ListInsert_Pro(4, FourthStepCode, ")");
            No++;
            if (ThirdStepCode->next) { toSet(ThirdStepCode, &fuc, &Begin, &End); }
            else { break; }
            ...
            ListInsert_Pro(4, FourthStepCode, _No);
            ListInsert_Pro(4, FourthStepCode, "]");
            ListInsert_Pro(4, FourthStepCode, "(in,/,");
            ListInsert_Pro(4, FourthStepCode, fuc);
            ListInsert_Pro(4, FourthStepCode, ")");
            No++;
            if (ThirdStepCode->next) { toSet(ThirdStepCode, &fuc, &Begin, &End); }
            else { break; }
            ...
            ListInsert_Pro(4, FourthStepCode, _No);
            ListInsert_Pro(4, FourthStepCode, "]");
            sprintf(all, "(out,/,%s)", toRegister("r[0]"));
            ListInsert_Pro(4, FourthStepCode, all);
            No++;
...
void genAssemblyCode(LinkList FourthStepCode,
                     FILE *asm_main) //就像高级语言可直接有解释器，之这样高级语言就是相对的目标代码，所以确实中间代码和目标代码都是相对的就像小公式和大公式之间的相对关系。这里其实就是第五步中间代码生成
{
    int ch;
    while (1) {
        ch = fgetc(asm_part[0]);
    ...
    while (1) {
        ...
        if (!strcmp(Parameter1, "in")) {
            fprintf(asm_main, "  call In\n  mov [Variable%d],eax\n", st_lookup(Parameter3));
        }
        ...
        else if (!strcmp(Parameter1, "jz")) {
            fprintf(asm_main, "  cmp %s,0\n  jz Address%s\n", Parameter2, Parameter3);
        }
        else if (!strcmp(Parameter1, "jnz")) {
            fprintf(asm_main, "  cmp %s,0\n  jnz Address%s\n", Parameter2, Parameter3);
        }
...
int main(int argc, char *argv[]) //当然最后main()还是要加入参数以操作不同的文件的
{
    ...
    genAssemblyCode(FourthStepCode, asm_main);
    DestroyList(&Track_Vector);
    DestroyList(&Terminal_Symbol);
    // DestroyList(&FirstStepCode); FirstStepCode其实就是Terminal_Symbol，之前有这个就一直段错误，己一度以为是fclose()的问题，这其实也是DestroyList实现不严谨的问题
