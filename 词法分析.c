...
int ZERO(FILE *source, FILE *intermediate_code) {
    ...
    } else if (isalpha(scanner)) {
        jump = 3;    //本来是isalnum()之以为就是判字母的，其实是判字母或数字，所以出bug后百思不得其解，这种bug很难发现。也像双重循环之本来是for(i=1;i<=100;i++)for(j=1;j<=100;j++)而错写成了for(i=1;i<=100;i++)for(j=1;i<=100;j++)之这样的错误比逻辑错误更难发现
        set[location++] = scanner;
        type = 1;
    } else if (isdigit(scanner)) {
        jump = 4;    //虽然case不能直接接判断条件而只能是常量，但可以直接给case接一个常量，然后执行if语句之巧妙模拟case直接接判断条件
        set[location++] = scanner;
        type = 2;
    } else {
        jump = 5;
        if (scanner != EOF) { set[location++] = scanner; }
    }
    return 0;
}
...
int (*state[])(FILE *source, FILE *intermediate_code)= {ZERO, ONE, TWO, THREE, FOUR,
                                                        FIVE}; //不用函数指针就是直接在状态结函数中调用其他状态结函数了，用函数指针更灵活之广义变量把离散元素编制化处理
