...
Status ListInsert_Pro(int n, LinkList *L, const char *str) //与形参改为char str[]而实参传入"abcd"也是可以的
{
    int i;
...
Status ListDelete(LinkList *L) {
    ...
    LinkList q = p->next;
    p->next = q->next;//p->next=p->next->next;//删除并释放结点
    free(q);
    return OK;
}
...
Status DestroyBucketList(BucketList *L) {   /* 初始条件：线性表L已存在。操作结果：销毁线性表L */
    BucketList q;
    if (*L == NULL)return 1;
    while (*L) {
        q = (*L)->next;//这样写是说明->的用法
